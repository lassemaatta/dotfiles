...
#+BEGIN_SRC emacs-lisp
  (setq
   ...
   version-control t
   ;; put lock files elsewhere
   lock-file-name-transforms'(("\\`/.*/\\([^/]+\\)\\'" "~/.emacs.d/aux/\\1" t)))
#+END_SRC
...
#+begin_src emacs-lisp
  (use-package jinx
    :ensure (:host github :repo "minad/jinx" :ref "9c71f2c896c3d004019aa45cff3a54e09f2d5aee")
  ...
  (use-package flycheck
    ...
    (defun my-flycheck-eldoc (callback &rest _ignored)
      ...
      (when-let ((flycheck-errors (and flycheck-mode (flycheck-overlay-errors-at (point)))))
        (mapc
         ...
         flycheck-errors)))
    (add-to-list 'eldoc-documentation-functions #'my-flycheck-eldoc t)
    ...
    (defun my-window-change-fn (window)
      ...
      (when (not flycheck-current-errors)
        (my-close-error-list)))
    (defun my-after-flycheck-change-fn ()
      (add-to-list 'window-buffer-change-functions #'my-window-change-fn)
      (if flycheck-current-errors
          (flycheck-list-errors)
        (my-close-error-list)))
    (add-hook 'flycheck-after-syntax-check-hook #'my-after-flycheck-change-fn))
#+end_src
...
#+begin_src emacs-lisp
  (use-package cov
    ...
    :after (projectile)
    :init
    (defun my-find-lcov (file-dir file-name)
      (let ((root (projectile-project-root)))
        (concat root "lcov.info")))
    :config
    (add-to-list 'cov-lcov-patterns #'my-find-lcov)
    (modus-themes-with-colors
      (custom-set-faces
       `(cov-none-face ((,c :foreground ,red-intense
                            :background ,bg-main)))
       `(cov-light-face ((,c :foreground ,yellow
                             :background ,bg-main)))
       `(cov-med-face ((,c :foreground ,green
                           :background ,bg-main)))
       `(cov-heavy-face ((,c :foreground ,green-intense
                             :background ,bg-main))))))
#+end_src
...
#+begin_src emacs-lisp
  (use-package eglot
    ...
    (defun my-eglot-loaded ()
      ...
      (when my-prefer-lsp-completion-at-point
...
#+BEGIN_SRC emacs-lisp
  (use-package lsp-mode
    ...
    (lsp-log-io nil)
    (lsp-xml-jar-download-url "https://www.eclipse.org/downloads/download.php?file=/lemminx/releases/0.29.0/org.eclipse.lemminx-uber.jar&mirror_id=1045")
    ...
    :hook (;; replace XXX-mode with concrete major-mode(e. g. python-mode)
           ...
           (lsp-completion-mode . my-refresh-clojure-setup)
...
#+begin_src emacs-lisp
  (use-package lsp-tailwindcss
    ...
    :custom
    (lsp-tailwindcss-major-modes '(web-mode clojurescript-mode))
    (lsp-tailwindcss-experimental-class-regex [":class\\s+\"([^\"]*)\""]))
#+end_src
...
#+BEGIN_SRC emacs-lisp
  (use-package cider
    ...
    (defun my-cider-eval (cmd)
      (cider-interactive-eval cmd nil nil (cider--nrepl-pr-request-plist)))
        ...
        (if (or (string-suffix-p "-test" ns)
          ...
          (let ((namespaces (cider-sync-request:ns-list))
            ...
            (cond
             ...
             (t unit-test-ns))))))
    :hook
    (clojurescript-mode . my-refresh-clojure-setup)
    (clojure-mode . my-refresh-clojure-setup)
    (cider-mode . my-refresh-clojure-setup)
    :custom
...
**** Completion (et al.) setup

Functions for managing =eldoc=, completions, and =xref= in Clojure(Script) buffers.

#+begin_src emacs-lisp
  (defun my-disable-cider-completion ()
    (when (bound-and-true-p cider-mode)
      (message "Disabling cider completion")
      (remove-hook 'completion-at-point-functions #'cider-complete-at-point)))

  (defun my-disable-cider-eldoc ()
    (when (bound-and-true-p cider-mode)
      (message "Disabling cider eldoc")
      (delete #'cider-eldoc eldoc-documentation-functions)))

  (defun my-enable-lsp-completion ()
    (when (bound-and-true-p lsp-completion-mode)
      (message "Enabling LSP completion")
      ;; First delete so that hooks are added last
      (remove-hook 'completion-at-point-functions #'lsp-completion-at-point)
      (remove-hook 'completion-at-point-functions #'my-eglot-completion-at-point)
      (if my-use-eglot
          (add-to-list 'completion-at-point-functions #'my-eglot-completion-at-point t)
        (add-to-list 'completion-at-point-functions #'lsp-completion-at-point t))))

  (defun my-disable-lsp-completion ()
    (message "Disabling LSP completion")
    (if my-use-eglot
        (remove-hook 'completion-at-point-functions #'my-eglot-completion-at-point )
      (remove-hook 'completion-at-point-functions #'lsp-completion-at-point )))

  (defun my-disable-lsp-eldoc ()
    (when (bound-and-true-p lsp-mode)
      (message "Disabling LSP eldoc")
      (if my-use-eglot
          (delete #'eglot-signature-eldoc-function eldoc-documentation-functions)
        (delete #'lsp-eldoc-function eldoc-documentation-functions))))

  (defun my-enable-lsp-eldoc ()
    (when (bound-and-true-p lsp-mode)
      (message "Enabling LSP eldoc")
      (if my-use-eglot
          (add-to-list 'eldoc-documentation-functions #'eglot-signature-eldoc-function t)
        (add-to-list 'eldoc-documentation-functions #'lsp-eldoc-function t))
      (add-to-list 'eldoc-documentation-functions #'my-flycheck-eldoc t)))

  (defun my-refresh-cljs-setup ()
    (message "Refreshing CLJS setup")
    ;; Never use cider completion, use LSP/eglot
    (my-disable-cider-completion)
    ;; Never use cider for eglot, use LSP/eglot
    (my-disable-cider-eldoc))

  (defun my-refresh-clj-setup ()
    (message "Refreshing CLJ setup")

    (if my-prefer-lsp-completion-at-point
        (progn
          ;; Make sure LSP completion is enabled if cider is shut down
          (my-disable-cider-completion)
          (my-enable-lsp-completion))
      (my-disable-lsp-completion))

    ;; Prefer cider eldoc over LSP/eglot while cider is runnign
    (if (bound-and-true-p cider-mode)
        (my-disable-lsp-eldoc)
      (my-enable-lsp-eldoc)))

  (defun my-refresh-clojure-setup ()
    (interactive)

    ;; Prefer LSP xref backend
    (when (bound-and-true-p cider-mode)
      (setq xref-backend-functions (remove 'cider--xref-backend xref-backend-functions)))

    (if (eq major-mode 'clojurescript-mode)
        (my-refresh-cljs-setup)
      (my-refresh-clj-setup)))
#+end_src
